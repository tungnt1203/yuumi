...
- The Docker sandbox runs only on the `--internal` network `yuumi-sandbox`. Its only way out is the egress proxy (`internal/egress`, container `yuumi-egress`), which allows `CONNECT :443` to `egress.DefaultAllowedHosts` only. A new outbound dependency of a review step must be added to that allowlist, or it fails with a `DENY` line in `docker logs yuumi-egress`. Do not forward the server's `HTTP(S)_PROXY` into the sandbox (it would bypass the egress proxy).
