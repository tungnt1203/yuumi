...
func (f *fakeGitHubClient) GetFileContent(repoFullName string, path string, ref string) ([]byte, bool, error) {
	...
	content, ok := f.baseFiles[ref+":"+path]
	if !ok {
...
func TestJobRun_LogsEachBundleReview(t *testing.T) {
	...
	// Không có lượt sửa định dạng nữa (issue #72): văn xuôi chỉ tốn đúng 1
	// lần gọi.
...
// finding thật. Header không được mở đầu bằng "✅ không có vấn đề".
func TestJobRun_PartialEmpty_HeaderDoesNotClaimClean(t *testing.T) {
	...
	if len(reviewer.prompts) != 2 {
		t.Fatalf("expected one Reviewer call per bundle, got %d", len(reviewer.prompts))
	}
	if !store.setCalled || store.gotSetSHA != "abc123" {
		t.Errorf("a prose bundle is not an error and should still save the reviewed SHA, setCalled=%v sha=%q", store.setCalled, store.gotSetSHA)
	}
}

// TestJobRun_StructuredOutput_DropsInvalidFinding: kết quả dạng structured
// output của CLI được đọc thẳng, và 1 finding hỏng chỉ bị bỏ riêng nó
// (issue #72).
func TestJobRun_StructuredOutput_DropsInvalidFinding(t *testing.T) {
	result := `{"findings":[{"category":"bug","severity":"high","message":"thiếu kiểm tra lỗi"},{"category":"bug","severity":"low","message":""}]}`
	...
	if len(reviewer.gotPrompts) != 1 {
		t.Errorf("expected exactly one Reviewer call, got %d", len(reviewer.gotPrompts))
	}
	if !strings.Contains(gh.editedBody, "thiếu kiểm tra lỗi") || !strings.Contains(gh.editedBody, "Tổng: 1 góp ý") {
		t.Errorf("expected the valid finding to be rendered and counted, got: %s", gh.editedBody)
	}
	if strings.Contains(gh.editedBody, "chưa đếm") || strings.Contains(gh.editedBody, "chưa đủ để kết luận") {
		t.Errorf("dropping one bad finding should not make the review partial, got: %s", gh.editedBody)
	}
}

func TestJobRun_BlankOutput_NotReportedClean(t *testing.T) {
	gh := &fakeGitHubClient{headSHA: "abc123", diff: "diff --git a/main.go b/main.go\n+fmt.Println(1)"}
